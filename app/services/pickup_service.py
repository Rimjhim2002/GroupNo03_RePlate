...
async def add_pickup_slot(
    food_listing_id: str, start_time: datetime, end_time: datetime, capacity: int, restaurant: User
) -> FoodListing:
    """Restaurant defines a pickup window for a listing they own."""
    food_listing = await FoodListing.get(food_listing_id)
    if food_listing is None:
        raise ValueError("Food listing not found")
    if food_listing.restaurant.ref.id != restaurant.id:
